...
<div class="coverart">
    <div class="coverart__artwork" style="width:${size};max-width:${size};height:${size};max-height:${size};cursor:pointer;<c:if test='${param.hideOverflow}'>overflow:hidden</c:if>;" title="${param.caption1}" id="${divId}">
        ...
        <div style="position: relative; width: 0; height: 0">
            <img class="playlist-keeper" alt="Play overlay" src="<spring:theme code='playOverlayImage'/>" id="${playId}"
                 style="position: relative; top: 8px; left: 8px; z-index: 2; display:none" >
        </div>

        <c:if test="${not empty param.albumId}">
          <div style="position: relative; width: 0; height: 0">
              <img class="playlist-keeper" alt="Add overlay" src="<spring:theme code='addOverlayImage'/>" id="${addId}"
    ...
    <c:if test="${captionCount gt 1}">
        <div class="caption2" style="width:${param.coverArtSize - 16}px"><a href="/main.view?id=${param.artistId}"title="${param.caption2}">${param.caption2}</a></div>
    </c:if>
